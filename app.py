...
@app.get("/api/health")
def health():
    return jsonify({"ready": data.is_ready, "candidates": len(data.candidates)})


@app.post("/api/identify")
def identify_api():
    """Camera/mobile endpoint: multipart form data with an `image` file."""
    image = request.files.get("image")
    if not image or not image.filename:
    ...
    predictions = recognizer.predict(UPLOAD_DIR / file_name)
    if not predictions:
        return jsonify({"error": "The vision model could not identify this image."}), 422
    return jsonify({"pokemon": data.details(predictions[0]["identifier"]), "predictions": predictions})
...
if __name__ == "__main__":
    # A phone on the same Wi-Fi network needs to reach this development server.
